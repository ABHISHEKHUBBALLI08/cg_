...
def high_score_display(high_score, dis):
    ...
    while not game_over:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                game_over = True
                game_close = False  # Ensure both flags are set to exit the loop
            if event.type == pygame.KEYDOWN:
        ...
        your_score(score, dis)
        high_score_display(high_score, dis)
...
def main():
    high_score = 0
    while True:
        high_score = gameLoop(high_score, dis)
